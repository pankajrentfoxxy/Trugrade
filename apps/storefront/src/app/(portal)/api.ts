 ...
 * Team invites
 *
 * `TeamInviteService`'s response types
 * (`apps/api/src/modules/identity/internal/team-invite.service.ts`), copied.
 * The invite routes are the same ones the supplier hub uses; the server reads
 * which side of the marketplace the caller is on and offers the roles for it.
 * ======================================================================== */
...
export interface TeamInvite {
  ...
  sentAt: string;
  expiresAt: string;
  /** Seconds left, measured by the server at read time. */
  expiresInSeconds: number;
}

/** `Team` plus the invites an owner may see. Empty for every other seat. */
export interface TeamWithInvites extends Team {
  invites?: TeamInvite[];
}

export const getTeamWithInvites = (): Promise<ApiResult<TeamWithInvites>> =>
  call<TeamWithInvites>('/api/account/team', { method: 'GET' });
...
export const createInvite = (body: {
  ...
  mobile: string;
  role: string;
}): Promise<ApiResult<{ invite: TeamInvite; emailSent: boolean }>> =>
  call('/api/account/team/invites', {
    method: 'POST',
    body: JSON.stringify({ ...body, facilityIds: [] }),
  });

export const resendInvite = (inviteId: string): Promise<ApiResult<TeamInvite>> =>
  call<TeamInvite>(`/api/account/team/invites/${inviteId}/resend`, { method: 'POST' });
...
export const previewInvite = (token: string): Promise<ApiResult<InvitePreview>> =>
  call<InvitePreview>(`/api/account/team/invites/preview?token=${encodeURIComponent(token)}`, {
    method: 'GET',
  });

/** No password: a buyer's invitee signs in with a code from then on. */
export const acceptInvite = (
  token: string,
): Promise<ApiResult<{ mfaRequired: boolean; orgType: string }>> =>
  call('/api/account/team/invites/accept', { method: 'POST', body: JSON.stringify({ token }) });
