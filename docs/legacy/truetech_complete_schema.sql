...
-- ==========================================================================
-- 13. DATABASE ROLES  (boundaries enforced by Postgres, not just by code)
-- ==========================================================================
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'tt_app') THEN
    CREATE ROLE tt_app LOGIN PASSWORD 'CHANGE_ME_IN_PRODUCTION';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'tt_readonly') THEN
    CREATE ROLE tt_readonly LOGIN PASSWORD 'CHANGE_ME_IN_PRODUCTION';
  END IF;
END $$;

GRANT USAGE ON SCHEMA identity, customer, vendor, kyc, catalog,
                       listing, ordering, qc, logistics, payment, platform TO tt_app;
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA
  identity, customer, vendor, kyc, catalog, listing, ordering, qc, logistics, payment, platform
  TO tt_app;
GRANT USAGE ON ALL SEQUENCES IN SCHEMA
  identity, customer, vendor, kyc, catalog, listing, ordering, qc, logistics, payment, platform
  TO tt_app;

-- Append-only tables: revoke UPDATE and DELETE even from the application role.
REVOKE UPDATE, DELETE ON identity.audit_log        FROM tt_app;
REVOKE UPDATE, DELETE ON payment.ledger_entry      FROM tt_app;
REVOKE UPDATE, DELETE ON listing.stock_movement    FROM tt_app;
REVOKE UPDATE, DELETE ON logistics.custody_event   FROM tt_app;
REVOKE UPDATE, DELETE ON kyc.consent_record        FROM tt_app;

-- Reporting role: read-only, and never sees encrypted PII columns.
GRANT USAGE ON SCHEMA identity, customer, vendor, catalog, listing,
                       ordering, qc, logistics, payment, platform TO tt_readonly;
GRANT SELECT ON ALL TABLES IN SCHEMA
  identity, customer, vendor, catalog, listing, ordering, qc, logistics, payment, platform
  TO tt_readonly;
REVOKE ALL ON kyc.pan_record, kyc.bank_account FROM tt_readonly;
