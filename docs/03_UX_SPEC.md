...
| `/account/orders/[id]/sales-order` | The booking as the dispatch points confirmed it: quantity ordered vs confirmed per line, price, GST, freight, total, and the payment. | All buyer roles | Pay (once online payment is connected) | `ordering.order`, `sub_order`, `order_line` | **Waiting:** until every dispatch point has answered — "waiting for the dispatch points to confirm", with how many have; no totals of a partly answered order. **Cancelled:** every dispatch point refused — nothing owed. | Amounts are for the confirmed quantity only. Credit-terms orders show no pay control. The word "vendor" never appears; a dispatch point is `Supply Point F · Noida`. |
