 ...
 *
 * The one rule: the dot is always signal blue, and signal blue never appears
...
/**
 * The lockup. `tru` in ink, `grade` in signal blue — and the brand string comes
...
export function Wordmark({ className }: { className?: string }): React.JSX.Element {
  ...
  return (
    <span className={cn('font-display text-h2 font-bold tracking-[-0.045em] lowercase', className)}>
      ...
      <span className="text-signal">{name.slice(split)}</span>
    </span>
  );
}
